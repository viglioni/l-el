...
- =:instance_of= - cl-typep (matches specific struct or class type)
- =:list_of= - l--list-of-p (matches list where every element matches a type keyword)
- =:list_of_instances= - l--list-of-instances-p (matches list where every element is an instance of a specific type)
