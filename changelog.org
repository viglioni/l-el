...
- Enhanced =l-mode= syntax highlighting:
  - =ldef= function names now highlighted with =font-lock-keyword-face= (bold purple/magenta)
  - Makes function definitions highly visible and easy to spot in code
  - Both the =ldef= keyword and function name use the same prominent highlighting
