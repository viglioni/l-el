...
** Added
- Typeclass system =l-typeclasses=: =lsemigroup= / =lmonoid= for
  =:string=, =:list=, =:vector=.  Operators =l<>=, =lempty=,
  =lmappend=; predicates =lsemigroup-p=, =lmonoid-p=.
- Class-hierarchy =ldef= dispatch: methods on EIEIO classes or
  =cl-defstruct= types match subclass instances; the most-specific
  class wins via runtime CPL, including under multiple inheritance.
- Depth-aware keyword specificity: =:natural= beats =:integer= beats
  =:number= when methods compete at the same tier.
- =li= macro for interactive arrow-syntax lambdas.

** Changed
- Internal load-path bootstrap recurses into every directory under
  =lib/=.
- Refactor: =l-subtype-p= reimplemented over a centralized hierarchy
  walk.  Behaviour unchanged.
- Refactor: arrow parsing in =l= and =ldef= extracted to =l--with-arrow=
  helper.  Behaviour unchanged.
- =load= no longer honours an end-of-file ~;; Local Variables:
  l-syntax: t~ form when deciding whether to apply the syntax
  transformation; only the prop-line (~-*- l-syntax: t -*-~) is
  considered for files loaded from disk.  Eval-side advices
  (=eval-buffer=, =eval-region=, =eval-last-sexp=) still honour both.
- Documented in the =l-syntax= defcustom that files declaring
  =l-syntax: t= must be loaded from source; =load-read-function= is
  not consulted for =.elc=.

** Fixed
- Errors from =l-require=, =l-syntax=, and unknown-type dispatch now
  signal typed conditions (=l-missing-library-error=,
  =l-invalid-syntax-error=, =l-unknown-type-predicate-error=) under
  =l-error= instead of plain =(error ...)=, so callers can catch them.
- l-syntax load no longer bypasses emacs's load pipeline.  Files used
  to be read whole, wrapped once in =with-l=, and =eval='d, which
  broke =features-being-required= (manifesting as ~Recursive 'require'
  for feature 'magit'~ under =use-package=), =load-history=,
  =load-file-name=, =current-load-list=, and =after-load-functions=.
  The =load= advice and =l-require= now install a =load-read-function=
  that transforms each top-level form and delegate to the real
  =load=, so the full pipeline runs unchanged.
- =eval-buffer= advice no longer fires when called from inside
  =load= (=load-with-code-conversion= invokes =eval-buffer=
  internally).  Previously the advice would read =current-buffer=
  (the user's outer buffer, not the file's load buffer), wrap that
  content in =with-l= and =eval= it -- producing the very
  load-pipeline bypass that broke =require= chains.  Guarded with
  =(not load-in-progress)= so loads always go through the standard
  read/eval loop; interactive =M-x eval-buffer= is unaffected.
- Class-hierarchy =ldef= dispatch no longer crashes on Emacs 30.
  =l--class-cpl-names= mapped =eieio-class-name= over
  =eieio--class-precedence-list=; on Emacs >= 30 the EIEIO CPL walks
  through =record= / =atom= / =t= built-in-class roots, which
  =eieio-class-name= rejects (~wrong-type-argument eieio--class~).
  Switched to =cl--class-name=, the accessor on the parent struct
  shared by =eieio--class=, =cl-defstruct= classes, and
  =built-in-class=.

** Internal
- Backfilled =;; since:= annotations on all top-level forms; corrected
  =;; updated-at:= lists to drop comment-only versions.
- CI workflow tweak silencing a warning.

* 1.1.2 - 2026-05-15

** Fixed
- Removed broken =(l-require 'typeclasses)= from =l.el= (module was
  never shipped).

** Internal
- Smoke test (=test/l-test.el=) loads =l.el= and pins =(l-version)= to
  the current release, so future top-level breakage fails CI.
- =scripts/release.el= bumps the smoke-test assertion alongside the
  =l-version= defun.

* 1.1.1 - 2026-05-14

** Added
- =:natural= type for positive integers; rejects 0, negatives, and floats.

** Changed
- Refactor: =ldef= primitive-vs-category specificity now derived from a
  new internal =l-type-hierarchy= data structure (replacing hardcoded
  =l-generic-primitive-types= / =l-generic-category-types= lists).
  Behaviour unchanged; =:integer= moved to category (parent of =:natural=).

** Removed
- =l-generic-primitive-types= and =l-generic-category-types= defvars
  (replaced by =l-type-hierarchy=).

** Fixed
- =l-mode= font-lock no longer freezes Emacs on certain source files.

** Internal
- File-level =;; since:= and =;; updated-at:= header convention added to
  all in-scope =.el= files.
- =scripts/release.el= rewritten with safety improvements (clean-tree
  precondition, explicit file list for =git add=, exit-code checks).
- CI: macOS Intel matrix =macos-13= → =macos-15-intel=.
